...
export class ValidatorService {
  ...
  isEqual(target: FormControl | AbstractControl): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      if (control.value) {
        return target.value !== control.value ? { equal: { value: true } } : null;
      } else {
        return null;
      }
    };
  ...
  isGreaterThan(target: FormControl | AbstractControl): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      if (control.value && typeof target.value === 'number') {
        return target.value > control.value ? { greater: { value: true } } : null;
      } else {
        return null;
      }
    };
  ...
  isLessThan(target: FormControl | AbstractControl): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      if (control.value && typeof target.value === 'number') {
        return target.value < control.value ? { less: { value: true } } : null;
      } else {
        return null;
      }
    };
  ...
  checkEmails(separator?: string): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      if (control.value) {
        let emails = control.value.split(separator ? separator : ',');
        let ctrl = new FormControl('', [Validators.required, Validators.email]);
